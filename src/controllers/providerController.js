...
exports.getAvailableJobs = async (req, res) => {
    ...
    const bookings = await Booking.find({ status: 'pending', provider: null, service: { $in: services.map((s) => s._id) } })
        .populate('service').populate('customer', 'name').sort({ scheduledDate: 1 }).limit(100);
